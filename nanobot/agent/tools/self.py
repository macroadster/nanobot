...
def _is_stream_monitor_status_snapshot(value: object) -> TypeGuard[Mapping[str, object]]:
    if not isinstance(value, Mapping):
        return False
    return all(field in value for field in ("monitor_id", "label", "phase"))
...
class MyTool(Tool):
    ...
    def _format_value(val: Any, key: str = "") -> str:
        ...
        if _is_stream_monitor_status(val) or _is_stream_monitor_status_snapshot(val):
            if _is_stream_monitor_status(val):
                monitor_id = val.monitor_id
                label = val.label
                phase = val.phase
                events_dispatched = val.events_dispatched
                events_seen = val.events_seen
                last_event_at = val.last_event_at
            else:
                monitor_id = val.get("monitor_id", "?")
                label = val.get("label", "?")
                phase = val.get("phase", "unknown")
                events_dispatched = val.get("events_dispatched", 0)
                events_seen = val.get("events_seen", 0)
                last_event_at = val.get("last_event_at")
            last = (
                f"{time.monotonic() - last_event_at:.0f}s ago"
                if isinstance(last_event_at, (int, float))
                else "none"
            )
            return (
                f"Monitor [{monitor_id}] '{label}' "
                f"phase={phase} events={events_dispatched}/{events_seen} "
                f"last={last}"
            )
