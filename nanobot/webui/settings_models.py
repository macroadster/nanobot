...
_OAUTH_PROXY_PROVIDERS = {"openai_codex", "xai_grok", "grok"}
...
def _provider_advanced_field_names(name: str, spec: Any) -> list[str]:
    fields: list[str] = []
    if spec.backend in {"openai_compat", "anthropic", "grok"}:
        fields.append("extra_headers")
    if spec.backend in {"openai_compat", "bedrock", "openai_codex", "xai_grok", "grok"}:
        fields.append("extra_body")
    if spec.backend in {"openai_compat", "grok"}:
...
def logout_oauth_provider(
    ...
    *,
    ...
    if spec.name == "openai_codex":
        ...
        if GROK_AUTH_FILE.exists():
            raw = _load_raw_auth_file() or {}
